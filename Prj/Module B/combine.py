...
optimizer1 = optim.SGD(net1.parameters(), lr=opt.lr, momentum=0.9, weight_decay=5e-4)
optimizer2 = optim.SGD(net2.parameters(), lr=opt.lr, momentum=0.9, weight_decay=5e-4)
...
def train(epoch):
    ...
    if epoch > learning_rate_decay_start and learning_rate_decay_start >= 0:
        ...
        utils.set_lr(optimizer1, current_lr)  # set the decayed rate
        utils.set_lr(optimizer2, current_lr)
    else:
        current_lr = opt.lr
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss.backward()
        utils.clip_gradient(optimizer1, 0.1)
        utils.clip_gradient(optimizer2, 0.1)
        optimizer1.step()
        optimizer2.step()
    ...
    f.write("Epoch:%d \t lr:%s \n " %(epoch,str(current_lr)))
...
def PublicTest(epoch):
    ...
    if PublicTest_acc > best_PublicTest_acc:
        ...
        state = {
            'net1': net1.state_dict() if use_cuda else net1,
            'net2': net1.state_dict() if use_cuda else net2,
            'acc': PublicTest_acc,
            'epoch': epoch,
        }
